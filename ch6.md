...
该指令将`a0`中的一个word加载到标签`var x`指向的内存中。与load global指令类似，标签的地址可能无法编码到指令中，因此汇编器可能生成多个RV32I机器指令，以便将标签指向的地址加载到寄存器中。此时，汇编程序不能使用`rs`（即`var_x`）作为临时寄存器，因为这样会改变`rs`中的内容。为此，用户需要显式指定一个通用寄存器，以便汇编器在为伪指令生成代码时将其用作临时寄存器。
