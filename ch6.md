...
流程控制指令是能够改变正常执行流的指令。在这种情况下，要执行的下一条指令取决于控制流指令的含义。
...
**间接流程控制指令是一种通过内存或寄存器，间接指定目标地址的控制流指令**。例如，`jr rs1`是一个间接流程控制指令，它跳转到的地址由寄存器`rs1`的值间接定义。因此改变`rs1`的值，可以很容易地改变指令的目标地址。
