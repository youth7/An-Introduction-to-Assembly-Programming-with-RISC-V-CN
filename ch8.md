...
* 使用`rt`指令从例程返回。该指令跳转到存储在`ra`寄存器中的地址。
* 按照RISC-V ilp32 ABI规范进行参数访问。对长度<=32位的标量参数，前8个参数在寄存器`a0`~`a7`中，其余的在堆栈上。此外，小于32位的整数标量（例如`char`或`short`）使用符号扩展到32位。
* 在栈上传递的参数被组织成这样：最后一个参数（即第n个参数）首先被压栈，第9个参数最后压栈。因此在例程开始的时候，`sp`指向第9个参数，`sp`+4指向第10个参数，以此类推。参数由caller例程在程序栈上分配，也必须由caller释放。callee不能释放由caller分配的参数；
* 如果例程需要在程序栈上存储信息，则应在例程开始时分配栈帧，并在返回之前释放栈帧。按照ABI的要求，栈帧的大小必须是16的倍数，以确保栈指针是128位对齐的（相对于程序的入口地址）。
* 例程可以使用寄存器来实现其功能，但被例程修改的callee-saved寄存器必须在例程开始时保存，并在从例程返回之前恢复。这些寄存器必须保存在栈帧上。
* caller可以修改和使用caller-saved寄存器而不保存它们，但是，如果需要在调用点之后使用这些寄存器，例程必须在调用点前后对其进行保存和恢复。caller必须保存和恢复`ra`，以便在调用点前后使用其内容。这些寄存器必须保存在栈帧上。
* 局部变量可以在寄存器或内存上分配。需要在内存上分配的局部变量必须分配到栈帧上。
* 可选地，`fp`可以用来保存一个指向栈帧开始的指针，提供了一个访问参数和局部变量的引用。此时，从例程返回时必须保持前一个帧指针（的值不变），因此，`fp`的值必须保存在函数开始时的栈帧中，并在返回之前恢复。
